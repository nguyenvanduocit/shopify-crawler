	...
	return func(msg *message.Message) (events []*message.Message, err error) {
		defer func() {
			if recove := recover(); recove != nil {
				if r.OnFailed != nil {
					_, err = r.OnFailed(msg, errors.WithStack(fmt.Errorf("recover: %v: ; Stack: %s", recove, debug.Stack())))
				} else {
					err = errors.WithStack(fmt.Errorf("recover: %v: ; Stack: %v", recove, string(debug.Stack())))
				}
			}
		}()
